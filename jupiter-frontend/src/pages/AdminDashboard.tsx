                    ...
                    <svg viewBox="0 0 380 200" className="admin-chart-svg">
                      ...
                      {(() => {
                        ...
                        return categories.map((cat, i) => {
                          ...
                          const matchingProducts = products.filter(p => (p.category || '').toLowerCase().includes(cat.key) || (p.name || '').toLowerCase().includes(cat.key)).length;
                          const val = matchingEnquiries > 0 ? (matchingEnquiries * 10) : Math.max(matchingProducts * 6, 12 - i * 2);
                          const barH = Math.min((val / 80) * 130, 130);
                          const barY = 160 - barH;
                          const x = 60 + i * 68;
                          return (
                            <g key={cat.name}>
                              <rect x={x - 14} y={barY} width="28" height={barH} rx="4" fill="#001827" />
                              <text x={x} y={barY - 6} fill="#001827" fontSize="10" fontWeight="700" textAnchor="middle">{val}</text>
                              {cat.name.split('\n').map((line, lineIdx) => (
                                <text key={lineIdx} x={x} y={178 + lineIdx * 10} fill="#64748B" fontSize="8" textAnchor="middle">{line}</text>
                              ))}
                            </g>
                          );
                        });
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                  {blogs.length > 0 && (
                    <button 
                      type="button" 
                      onClick={() => {
                        if (confirm('Are you sure you want to remove all blog articles?')) {
                          clearAllBlogs();
                          setBlogs([]);
                ...
                <div style={{ display: 'flex', gap: '10px' }}>
                  {gallery.length > 0 && (
                    <button 
                      type="button" 
                      onClick={() => {
                        if (confirm('Are you sure you want to remove all gallery photos?')) {
                          clearAllGalleryPhotos();
                          setGallery([]);
                        ...
                        <div>
                          ...
                          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '10px' }}>
                            ...
                            <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
                              ...
                              <button 
                                onClick={() => {
                                  if (window.confirm(`Remove machine delivery record for "${loc.clientName}"?`)) {
